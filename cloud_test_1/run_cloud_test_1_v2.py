...
import hashlib, json, math, os, re, subprocess, sys, tarfile, warnings
...
print("Resolving official source links...", flush=True)
links, readme_text = resolve_links()
missing = [f for f in EXPECTED if f not in links]
if missing:
    raise RuntimeError(f"Official README links not resolved for: {missing}")
...
for fn, expected in EXPECTED.items():
    ...
    if actual != expected:
        raise RuntimeError(f"Checksum failed: {fn}")
    with tarfile.open(p, "r:gz") as tar:
        tar.extractall(RAW)

(OUT / "source_registry.json").write_text(json.dumps(registry, indent=2))

job_path = RAW / "pai_job_table.csv"
task_path = RAW / "pai_task_table.csv"
if not job_path.exists() or not task_path.exists():
    raise RuntimeError("Expected CSV files were not extracted.")

JOB_COLUMNS = ["job_name", "inst_id", "user", "status", "start_time", "end_time"]
TASK_COLUMNS = ["job_name", "task_name", "inst_num", "status", "start_time", "end_time", "plan_cpu", "plan_mem", "plan_gpu", "gpu_type"]

print("Loading extracted CSV tables...", flush=True)
jobs = pd.read_csv(job_path, header=None, names=JOB_COLUMNS)
tasks = pd.read_csv(task_path, header=None, names=TASK_COLUMNS)
...
def ecdf(train, values):
    tr = np.sort(pd.Series(train).dropna().to_numpy(float))
    if len(tr) == 0:
        return pd.Series(np.nan, index=values.index)
    return pd.Series(values, index=values.index).map(
        lambda x: np.nan if pd.isna(x) else np.searchsorted(tr, x, side="right") / len(tr)
    )
...
wait90 = dev["wait_time"].quantile(.90)
...
bq = dev2["B"].quantile(.75)
pq = dev2["P"].quantile(.75)
eps = dev2["W"].quantile(.25)
d["B_norm"] = d["B"] / bq
d["P_norm"] = d["P"] / pq
...
print("Fitting comparator models...", flush=True)
...
ablations = {k: score_binary(v) for k,v in gate_preds.items()}
...
results = {
    ...
    "hard_MQ": score_binary(test["MQ"].to_numpy()),
    "continuous_H_star": score_prob(test["H_star"].to_numpy()),
    "logistic": score_prob(p_logit),
    "gradient_boosting": score_prob(p_gb),
    "components_AUROC": {
        "B": float(roc_auc_score(y,test["B"])),
        "P": float(roc_auc_score(y,test["P"])),
...
print("Writing result package...", flush=True)
...
receipt = {"seed":SEED, "source_registry_sha256":digest(OUT/"source_registry.json"),
           "panel_sha256":digest(OUT/"panel.csv"), "metrics_sha256":digest(OUT/"metrics.json")}
