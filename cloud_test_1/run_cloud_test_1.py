...
def load_year(year: int):
    ...
    if len(wide_map) == len(CONDS):
        ...
        return out, {"year": year, "status": "OK_WIDE", "rows": int(len(df)), "cols": int(len(original_cols))}
    ...
    for cond in CONDS:
        if cond not in wide.columns:
            wide[cond] = np.nan
    return wide[["facility_id", "year"] + CONDS], {"year": year, "status": "OK_LONG", "rows": int(len(df)), "matched_rows": int(len(tmp))}
...
def make_pairs(panel: pd.DataFrame, horizon: int) -> pd.DataFrame:
    ...
    p = cur.merge(future, on=["facility_id", "future_year"], how="inner")
    p["dY"] = p["Y_next"] - p["Y"]
    return p
...
def rolling(pairs: pd.DataFrame, horizon: int):
    ...
    for target_year in sorted(pairs["future_year"].dropna().astype(int).unique()):
        ...
        for model, feats in models.items():
            ...
            metrics.append({"horizon": horizon, "target_year": int(target_year), "model": model, "n_train": int(len(train.dropna(subset=feats+["Y_next"]))), "n_test": int(len(te)), "rmse": rmse, "mae": mae})
...
def main():
    ...
    panel = panel.merge(prev, on=["facility_id", "year"], how="left")
    ...
    findings = {"route_conditions": CONDS, "years_requested": YEARS, "source_receipts": receipts, "pair_summary": pair_summary, "horizons": {}}
    for h in (1,3):
